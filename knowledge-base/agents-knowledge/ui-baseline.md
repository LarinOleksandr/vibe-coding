...
- Default UI system: **shadcn/ui + Tailwind** (per `frontend/AGENTS.md` and `KB_TECH_STACK`).
...
- “Base UI primitives” (Button, Input, Dialog, etc.) are centralized and reused.
...
- Colors: use semantic tokens (e.g. “foreground”, “muted”, “destructive”), not raw color literals.
...
- One-off visual hacks to “make it look right” without adding/using tokens
- Copy-pasted variants (if it’s a variant, it must be expressed as a real variant)
...
**Date — Component/Area — Rule violated — Reason — Plan to remove — Target date**
