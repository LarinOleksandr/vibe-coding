# knowledge-base/agents-knowledge/deployment-and-cicd.md
...
- Use stack-defined test frameworks (see `KB_TECH_STACK`); do not add new test deps without explicit approval (see `KB_REPOSITORY_RULES`).
...
- Default to **Netlify** for a Vite SPA (matches current stack intent in `KB_TECH_STACK`).
