...
1. **Thin always-on context**  
   Only constraints, routing, and completion rules are always loaded.

2. **Single source of truth**  
   Each rule or concept lives in exactly one place.

3. **On-demand knowledge**  
   Large reference material is consulted only when routed.

4. **Procedures != rules**  
   Rules live in AGENTS.md; procedures live in Skills.

5. **Locality of constraints**  
   Rules apply at the closest directory level where they differ.
...
- Command lists or tool catalogs (live in knowledge-base/agents-knowledge/tools.md)
...
**Location:** Dedicated folder (e.g. `knowledge-base/agents-knowledge/`)
...
| Knowledge/reference    | knowledge-base/agents-knowledge |
| Procedures             | Skills                          |
...
- Use `$context-maintenance` for audits and updates.
- Follow `KB_CONTEXT_MAINTENANCE`.
