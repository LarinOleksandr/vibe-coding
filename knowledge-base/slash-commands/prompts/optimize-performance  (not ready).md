...
Conduct a thorough review covering the areas below, including dependency simplification and justification where relevant.
...
- Unused, redundant, or overlapping dependencies (simplify where possible)
- Heavy dependencies that could be replaced with lighter alternatives or native APIs
- Duplicate/transitive dependency bloat (multiple versions of the same package)
- Missing tree-shaking opportunities (CJS vs ESM, deep imports, sideEffects flags)
- Lack of bundle analysis or size budgets where bundle size impacts runtime performance
- Missing justification for each dependency (why needed, what it replaces, and cost/benefit)
